...
export default function VisitorDashboard() {
    ...
    const loadData = async () => {
        try {
            const response = await api.get(`/admin/visitors/dashboard?period=${period}`);
                    ...
                    <div className={`${classes.card} p-6 rounded-lg shadow`}>
                        ...
                        <ResponsiveContainer width="100%" height={300}>
                            <LineChart data={data?.daily}>
                                <CartesianGrid strokeDasharray="3 3" stroke={classes.border} />
                                <XAxis dataKey="date" stroke={classes.text} />
                                <YAxis stroke={classes.text} />
                                <Tooltip 
                                    contentStyle={{ backgroundColor: theme === 'dracula' ? '#282a36' : '#fff' }}
                                    labelStyle={{ color: classes.text }}
                                />
                                <Legend wrapperStyle={{ color: classes.text }} />
                    ...
                    <div className={`${classes.card} p-6 rounded-lg shadow`}>
                        ...
                        <ResponsiveContainer width="100%" height={300}>
                            <BarChart data={data?.byBrowser} layout="vertical">
                                <CartesianGrid strokeDasharray="3 3" stroke={classes.border} />
                                <XAxis type="number" stroke={classes.text} />
                                <YAxis type="category" dataKey="browser" stroke={classes.text} width={100} />
                                <Tooltip 
                                    contentStyle={{ backgroundColor: theme === 'dracula' ? '#282a36' : '#fff' }}
                                    labelStyle={{ color: classes.text }}
                                />
                    ...
                    <div className={`${classes.card} p-6 rounded-lg shadow`}>
                        ...
                        <ResponsiveContainer width="100%" height={300}>
                            <BarChart data={data?.byCountry} layout="vertical">
                                <CartesianGrid strokeDasharray="3 3" stroke={classes.border} />
                                <XAxis type="number" stroke={classes.text} />
                                <YAxis type="category" dataKey="country" stroke={classes.text} width={100} />
                                <Tooltip 
                                    contentStyle={{ backgroundColor: theme === 'dracula' ? '#282a36' : '#fff' }}
                                    labelStyle={{ color: classes.text }}
                                />
                ...
                <div className={`${classes.card} p-6 rounded-lg shadow mb-8`}>
                    ...
                    <div className="overflow-x-auto">
                        <table className="min-w-full">
                            <thead className={classes.border}>
                                <tr>
                                    <th className={`text-left py-2 ${classes.text}`}>Página</th>
                                    <th className={`text-right py-2 ${classes.text}`}>Visualizações</th>
                                    <th className={`text-right py-2 ${classes.text}`}>Visitantes Únicos</th>
                                </tr>
                            </thead>
                            <tbody>
                                {data?.topPages?.map(page => (
                                    <tr key={page.page_url} className="border-t">
                                        <td className={`py-2 ${classes.text}`}>{page.page_url || '/'}</td>
                                        <td className={`py-2 text-right ${classes.text}`}>{formatNumber(page.views)}</td>
                                        <td className={`py-2 text-right ${classes.text}`}>{formatNumber(page.unique_visitors)}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
                ...
                <div className={`${classes.card} p-6 rounded-lg shadow`}>
                    ...
                    <div className="overflow-x-auto">
                        <table className="min-w-full">
                            <thead className={classes.border}>
                                <tr>
                                    <th className={`text-left py-2 ${classes.text}`}>Navegador/OS</th>
                                    <th className={`text-left py-2 ${classes.text}`}>Localização</th>
                                    <th className={`text-left py-2 ${classes.text}`}>Última Página</th>
                                    <th className={`text-left py-2 ${classes.text}`}>Visitas</th>
                                    <th className={`text-left py-2 ${classes.text}`}>Último Acesso</th>
                                 </tr>
                            </thead>
                            <tbody>
                                {data?.recent?.map(visitor => (
                                    <tr key={visitor.id} className="border-t cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700"
                                        ...
                                        <td className={`py-2 ${classes.text}`}>{visitor.last_page || '-'}</td>
                                        <td className={`py-2 ${classes.text}`}>{visitor.visit_count}</td>
                                        <td className={`py-2 ${classes.text}`}>
                    ...
                    <div className={`${classes.card} rounded-lg p-6 w-[600px] max-h-[90vh] overflow-y-auto`}>
                        ...
                        <div className="space-y-3">
                            <div className="grid grid-cols-2 gap-2">
                                ...
                                <span className="font-mono text-sm">{selectedVisitor.session_id}</span>
                                ...
                                <span className="font-semibold">Convertido:</span>
                                <span>{selectedVisitor.converted ? '✅ Sim' : '❌ Não'}</span>
                            </div>
                        </div>
                        
                        <button
                            onClick={() => setSelectedVisitor(null)}
                            className={`w-full mt-6 py-2 rounded ${classes.button || 'bg-blue-600 text-white hover:bg-blue-700'}`}
                        >
                            Fechar
                        </button>
                    </div>
